...
import { VAULT_ORG_AWAKE_PATH } from "../constant.js";
...
export const getAwakeSections = async () => {
  const url = await getSectionUrl("awake");
  const html = await getLvPageAPI(url);
  return { sections: parseLvLinks(html) };
};

// Phase 1: 구조 크롤 + docId 매핑 등록
export const buildAwakeMappings = async (docidMap, selection = null) => {
  console.log("Collecting Awake! structure (mapping phase)...");
  ...
  const rootUrl = await getSectionUrl("awake");
  const siblingUrls = new Set([rootUrl, ...sectionsToProcess.map(s => s.url)]);

  for (const section of sectionsToProcess) {
    console.log(`\n[${section.title}] Scanning...`);
    const yearFolder = `${VAULT_ORG_AWAKE_PATH}${sanitizeFilename(section.title)}/`;
  ...
  console.log(`Awake! mappings registered: ${preparedSections.length} sections.`);
  return preparedSections;
};

// Phase 2: 콘텐츠 임포트
export const importAwake = async (listOfExistingFiles, docidMap, preparedSections) => {
  console.log("\nStarting Awake! content import...");
