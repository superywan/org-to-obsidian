...
import { VAULT_ORG_KINGDOM_SERVICE_PATH } from "../constant.js";
...
export const getKingdomServiceSections = async () => {
  const url = await getSectionUrl("kingdom-service");
  const html = await getLvPageAPI(url);
  return { sections: parseLvLinks(html) };
};

// Phase 1: 구조 크롤 + docId 매핑 등록
export const buildKingdomServiceMappings = async (docidMap, selection = null) => {
  console.log("Collecting Kingdom Service structure (mapping phase)...");
  ...
  const rootUrl = await getSectionUrl("kingdom-service");
  const siblingUrls = new Set([rootUrl, ...sectionsToProcess.map(s => s.url)]);

  for (const section of sectionsToProcess) {
    console.log(`\n[${section.title}] Scanning...`);
    const sectionFolder = `${VAULT_ORG_KINGDOM_SERVICE_PATH}${sanitizeFilename(section.title)}/`;
  ...
  console.log(`Kingdom Service mappings registered: ${preparedSections.length} sections.`);
  return preparedSections;
};

// Phase 2: 콘텐츠 임포트
export const importKingdomService = async (listOfExistingFiles, docidMap, preparedSections) => {
  console.log("\nStarting Kingdom Service content import...");
  ...
  console.log("\nKingdom Service import complete.");
};
