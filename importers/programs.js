...
import { VAULT_ORG_PROGRAMS_PATH } from "../constant.js";
...
export const getProgramSections = async () => {
  const url = await getSectionUrl("programs");
  const html = await getLvPageAPI(url);
  return { sections: parseLvLinks(html) };
};

// Phase 1: 구조 크롤 + docId 매핑 등록
export const buildProgramMappings = async (docidMap, selection = null) => {
  console.log("Collecting Programs structure (mapping phase)...");
  ...
  const rootUrl = await getSectionUrl("programs");
  const siblingUrls = new Set([rootUrl, ...sectionsToProcess.map(s => s.url)]);

  for (const section of sectionsToProcess) {
    console.log(`\n[${section.title}] Scanning...`);
    const sectionFolder = `${VAULT_ORG_PROGRAMS_PATH}${sanitizeFilename(section.title)}/`;
  ...
  console.log(`Program mappings registered: ${preparedSections.length} sections.`);
  return preparedSections;
};

// Phase 2: 콘텐츠 임포트
export const importPrograms = async (listOfExistingFiles, docidMap, preparedSections) => {
  console.log("\nStarting Programs content import...");
