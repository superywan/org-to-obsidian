...
import { VAULT_ORG_WEB_SERIES_PATH } from "../constant.js";
import {
  getLvPageAPI,
...
export const getWebSeriesSections = async () => {
  const sectionUrl = await getSectionUrl("web-series");
...
export const buildWebSeriesMappings = async (docidMap, pubSelection = null) => {
  console.log("Collecting Web Series from WOL (mapping phase)...");
  ...
  for (const pub of publications) {
    ...
    const pubFolder = `${VAULT_ORG_WEB_SERIES_PATH}${safeTitle}`;
  ...
  console.log(`Web Series mappings registered: ${preparedPubs.length} publications.`);
  return preparedPubs;
};

// Phase 2: 콘텐츠 임포트
export const importWebSeries = async (listOfExistingFiles, docidMap, preparedPubs) => {
  console.log(`\nStarting web series content import (${preparedPubs.length} publications)...`);
