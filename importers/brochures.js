...
import { VAULT_ORG_BROCHURES_PATH } from "../constant.js";
import {
  getLvPageAPI,
...
export const getBrochureSections = async () => {
  const sectionUrl = await getSectionUrl("brochures");
...
export const buildBrochureMappings = async (docidMap, pubSelection = null) => {
  console.log("Collecting Brochures from WOL (mapping phase)...");
  ...
  for (const pub of publications) {
    ...
    const pubFolder = `${VAULT_ORG_BROCHURES_PATH}${safeTitle}`;
  ...
  console.log(`Brochure mappings registered: ${preparedPubs.length} publications.`);
  return preparedPubs;
};

// Phase 2: 콘텐츠 임포트
export const importBrochures = async (listOfExistingFiles, docidMap, preparedPubs) => {
  console.log(`\nStarting brochure content import (${preparedPubs.length} publications)...`);
