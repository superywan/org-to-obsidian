...
- 실시간 로그 스트리밍 (최신 로그가 **맨 위**에 표시)
- **🗂️ 구조 캐시 사용** 토글 — 이전 Phase 1 크롤 결과를 재사용해 재크롤을 생략(아래 참고)
- 카테고리 카드의 **🔄 버튼** — 구조 캐시가 있어도 그 항목만 강제 재크롤(아래 참고)
...
- ✅ 재실행 시 구조 크롤 시간 절약
- ⚠️ 캐시를 쓰는 동안 **새로 나온 호수/기사는 놓칠 수 있음** → 아래 🔄로 해결
...
### 내 노트에 성구 태그 달기 (`tag-vault-folder.js`)

임포트로 만들어진 문서가 아니라, **직접 작성한 md 파일**에도 성경 구절 패턴이 나오면 임포터와 동일한 로직으로 성구 태그(+성경 wikilink)를 달아 주는 도구입니다.

```bash
# 볼트의 tagging/ 폴더를 미리보기 (파일 수정 안 함) — 먼저 확인 권장
node tag-vault-folder.js --dry-run

# 실제 적용
node tag-vault-folder.js

# 다른 폴더 지정 (볼트 기준 상대경로 또는 절대경로)
node tag-vault-folder.js "메모/성구노트"
```

동작 예시:

```
로마서 15:19  →  [[library/org-bible/…/로마서 15장#^v19|로마서 15:19]] #성구/로마서/15/19
...
- `docid-map`에 성경 장(`b:책:장`) 키가 있어야 wikilink가 생성되므로 **성경 임포트 완료 후 실행**하세요 (없으면 경고 후 태그만 추가)

> **참고:** 대상 폴더는 Vault라 git 추적 밖입니다. 되돌릴 안전망이 없으니 `--dry-run`으로 먼저 확인하세요.
